...
export function generateResourceMap({ map, cities, width, height, targetCount = DEFAULT_TARGET_COUNT }) {
  ...
  for (const city of cities) {
    const nearbyStrategic = city.owner === 'player' ? 'iron' : 'horses';
    const nearbyLuxury = city.owner === 'player' ? 'gems' : 'spices';
    placeNearCity({ map, width, height, city, defKey: nearbyStrategic, occupied, addResource });
    placeNearCity({ map, width, height, city, defKey: nearbyLuxury, occupied, addResource });
  }
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      if (occupied.has(keyFor(x, y)) || map[y][x] === 'water') continue;
      const score = hashResource(x, y, width, height);
  ...
  for (const candidate of candidates) {
    ...
    for (let i = 0; i < resourceKeys.length; i++) {
      if (addResource(resourceKeys[(start + i) % resourceKeys.length], candidate.x, candidate.y)) break;
    }
  }
...
export function collectEmpireResources({ owner, cities, map, resources, terrainYield, radius = 1 }) {
  const ownerCities = cities.filter(city => city.owner === owner);
  const controlled = [];
  const seen = new Set();
  const cityYields = [];
  const totals = { food: 0, prod: 0, sci: 0 };

  for (const city of ownerCities) {
    const cityTotal = { food: 0, prod: 0, sci: 0 };
    for (let dy = -radius; dy <= radius; dy++) {
      for (let dx = -radius; dx <= radius; dx++) {
        const x = city.x + dx;
        const y = city.y + dy;
        if (!map[y]?.[x]) continue;
        addYield(cityTotal, terrainYield[map[y][x]] ?? emptyYield());
        const resource = resourceAt(resources, x, y);
        if (!resource) continue;
        addYield(cityTotal, resource.yield);
    ...
    addYield(totals, cityTotal);
    cityYields.push({ city, yields: cityTotal });
  }

  const stock = {};
  for (const resource of controlled) {
